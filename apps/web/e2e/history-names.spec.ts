 ...
 * All-games view of Your record (deterministic DEMO_HISTORY) — and against
 * the real server a fresh identity sees the empty state on #history (which
 * soft-redirects into Your record).
 */

test('the All games list shows teammate and opponents by name', async ({ page }) => {
...
test('#history lands a fresh identity on the record empty state', async ({ browser }) => {
