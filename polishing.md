...
- The filter is **UX simplicity first**: refinements and cuts beat additions. When in
  doubt, an item should make the game feel *simpler*, faster, or more consistent.
...
- [ ] **(M) FR tutoiement pass on ALL unlock/requirement copy** — the single biggest
  ...
  gameplay surface says *tu*. One coordinated pass → "Gagne / Atteins / Réussis / Joue /
...
- [ ] (S) `PublicLobby.tsx` empty-state copy references Quick Play but renders *below*
...
- [ ] (S) `ScoreStrip.tsx:732` — the whose-turn "action" line (the novice's anchor) is the
  *smallest* text in the phone HUD (`max-sm:text-[0.55em]`). Bump to ≥0.62em.
